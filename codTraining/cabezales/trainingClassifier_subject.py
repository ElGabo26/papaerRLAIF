...
#bases  de  subject
bases=[x for x in  bases if 'subject' in x]
print(bases)
...
resultado.to_csv(f"{OUTPUT_MODEL}/metadata_subject_pooling.csv")
...
datos = torch.load(
            f"{DATA_ROUTE}/subject_{pooling}.pt",
            map_location="cpu",
            weights_only=True)
...
for seed in seeds:
    ...
    for batch in batchConfig:
        ...
        name=f"models_subject_seed_{seed}_batch_{batchConfig.index(batch) +1}"
...
finalBase.to_csv(f"{OUTPUT_MODEL}/subject_finalBase.csv")
