...
bases=[x for x in  bases if 'skill' in x]
print(bases)
    ...
    for db in bases:
        ...
        for seed in seeds:
            ...
            for hidden_dim in hidden_dim_options:
                ...
                model, data=train_eval_multiclass(DEVICE,NUM_EPOCHS,clasificador,criterio,optimizador,trainL,testL,3,MIN_DELTA)
...
resultado.to_csv(f"{OUTPUT_MODEL}/metadata_skill_pooling.csv")
...
datos = torch.load(
            f"{DATA_ROUTE}/skill_{pooling}.pt",
            map_location="cpu",
            weights_only=True)

input_dim=datos.tensors[0].shape[1]

configs=[x.update({"input_dim": input_dim, 'num_classes':NUM_CLASSES,"device": DEVICE})
         for x in configs]
...
for seed in seeds:
    ...
    for batch in batchConfig:
        ...
        name=f"models_skill_seed_{seed}_batch_{batchConfig.index(batch) +1}"
...
finalBase.to_csv(f"{OUTPUT_MODEL}/skill_finalBase.csv")
